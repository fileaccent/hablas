...
def calc_expect_func(m, lda, incx, uplo, trans, diag):
    input1 = np.random.uniform(1, 1, size=(m, lda,)).astype(np.float32)

    input2 = np.random.uniform(1, 1, size=(m * incx,)).astype(np.float32)
    ...
    if trans == 0 :
        y_ture = (np.matmul(input1_true.astype(np.float32), input2_true.astype(np.float32))).astype(np.float32) 
    else :
        y_ture = (np.matmul(input2_true.astype(np.float32), input1_true.astype(np.float32))).astype(np.float32) 
